...
RUN install -d ${JAVA_HOME} && \
    if [ "$(uname -m)" = "x86_64" ]; then \
      curl -L "${JAVA_JDK_BASE_URL}/jdk-${JAVA_VERSION}%2B10/OpenJDK${JAVA_MAJOR_VERSION}U-jdk_x64_linux_hotspot_${JAVA_VERSION}_${JAVA_BUILD_VERSION}.tar.gz" | tar xvz --strip-components=1 -C ${JAVA_HOME}; \
      ...
      curl -L "${JAVA_JDK_BASE_URL}/jdk-${JAVA_VERSION}%2B10/OpenJDK${JAVA_MAJOR_VERSION}U-jdk_aarch64_linux_hotspot_${JAVA_VERSION}_${JAVA_BUILD_VERSION}.tar.gz" | tar xvz --strip-components=1 -C ${JAVA_HOME}; \
      ...
      curl -L "${JAVA_JDK_BASE_URL}/jdk-${JAVA_VERSION}%2B10/OpenJDK${JAVA_MAJOR_VERSION}U-jdk_arm_linux_hotspot_${JAVA_VERSION}_${JAVA_BUILD_VERSION}.tar.gz" | tar xvz --strip-components=1 -C ${JAVA_HOME}; \
      curl -L "https://github.com/hadolint/hadolint/releases/download/v${HADOLINT_VERSION}/hadolint-Linux-arm64" --output /usr/local/bin/hadolint; \
    else \
